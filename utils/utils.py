...
def add_caption(image, text, position = "bottom-mid",  font = None, text_color= 'black', bg_color = (255, 255, 255) , bg_opacity = 200):
    ...
    for ind, line in enumerate(lines[::-1]):
        # text_width, text_height = draw.textlength(line, font=font)
        text_width = draw.textlength(line, font=font)
        text_height = draw.textlength(line, font=font)
...
def get_comic_classical(images,captions = None,font = None,pad_image = None):
    ...
    pad_image = pad_image.resize(images[0].size, Image.DEFAULT_STRATEGY)
...
def concat_images_vertically_and_scale(images,scale_factor=2):
    ...
    resized_image = concatenated_image.resize((new_width, new_height), Image.DEFAULT_STRATEGY)
...
def combine_images_vertically_with_resize(images):
    ...
    for img in images:
        ...
        # 调整图片大小
        resized_img = img.resize((min_width, new_height), Image.DEFAULT_STRATEGY)

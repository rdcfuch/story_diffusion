...
def get_image_path_list(folder_name):
    ...
    print("image_path: " + str(image_path_list))
...
pipe1 = StableDiffusionXLPipeline.from_pretrained(sd_model_path, torch_dtype=torch.float16,
...
######### Image Generation ##############
@spaces.GPU
def process_generation(_sd_type, _model_type, _upload_images, _num_steps, style_name, _Ip_Adapter_Strength,
                       _style_strength_ratio, guidance_scale, seed_, sa32_, sa64_, id_length_, general_prompt,
                       negative_prompt, prompt_array, G_height, G_width, _comic_type):
    ...
    prompts = [general_prompt + "," + prompt if "[NC]" not in prompt else prompt.replace("[NC]", "") for prompt in
               clipped_prompts]
    ...
    total_results = []
    if _model_type == "original":
        id_images = pipe(id_prompts, num_inference_steps=_num_steps, guidance_scale=guidance_scale, height=height,
                         width=width, negative_prompt=negative_prompt, generator=generator).images
    elif _model_type == "Photomaker":
        id_images = pipe(id_prompts, input_id_images=input_id_images, num_inference_steps=_num_steps,
                         guidance_scale=guidance_scale, start_merge_step=start_merge_step, height=height, width=width,
    ...
    for real_prompt in real_prompts:
        ...
        if _model_type == "original":
            real_images.append(
                pipe(real_prompt, num_inference_steps=_num_steps, guidance_scale=guidance_scale, height=height,
                     width=width, negative_prompt=negative_prompt, generator=generator).images[0])
        elif _model_type == "Photomaker":
            real_images.append(pipe(real_prompt, input_id_images=input_id_images, num_inference_steps=_num_steps,
                                    guidance_scale=guidance_scale, start_merge_step=start_merge_step, height=height,
                                    width=width, negative_prompt=negative_prompt, generator=generator).images[0])
        else:
            raise NotImplementedError("You should choice between original and Photomaker!",
    ...
    if _comic_type != "No typesetting (default)":
        ...
        total_results = get_comic(id_images + real_images, _comic_type, captions=captions,
                                  font=ImageFont.truetype("./fonts/Inkfree.ttf", int(45))) + total_results
    set_attention_processor(pipe.unet, id_length_, is_ipadapter=False)
    yield total_results
...
with gr.Blocks(css=css) as demo:
    ...
    with gr.Row():
        with gr.Group(elem_id="main-image"):
            ...
            with gr.Column(visible=True) as gen_prompt_vis:
                sd_type = gr.Dropdown(choices=list(models_dict.keys()), value="Unstable", label="sd_type",
                ...
                with gr.Group(visible=False) as control_image_input:
                    ...
                    with gr.Column(visible=False) as clear_button:
                        remove_and_reupload = gr.ClearButton(value="Remove and upload new ones", components=files,
                                                             size="sm")
                ...
                prompt_array = gr.Textbox(lines=3, value='',
                ...
                with gr.Accordion("(4) Tune the hyperparameters", open=True):
                    ...
                    sa64_ = gr.Slider(label=" (The degree of Paired Attention at 64 x 64 self-attention layers) ",
                                      minimum=0, maximum=1., value=0.7, step=0.1)
                    id_length_ = gr.Slider(label="Number of id images in total images", minimum=2, maximum=4, value=2,
                                           step=1)
                    ...
                    comic_type = gr.Radio(["No typesetting (default)", "Four Pannel", "Classic Comic Style"],
                                          value="Classic Comic Style", label="Typesetting Style",
        ...
        with gr.Column():
            ...
            gr.Markdown(version)
    model_type.change(fn=change_visiale_by_model_type, inputs=model_type,
    ...
    final_run_btn.click(fn=set_text_unfinished, outputs=generated_information
                        ).then(process_generation,
                               inputs=[sd_type, model_type, files, num_steps, style, Ip_Adapter_Strength,
                                       style_strength_ratio, guidance_scale, seed_, sa32_, sa64_, id_length_,
                                       general_prompt, negative_prompt, prompt_array, G_height, G_width, comic_type],
                               outputs=out_image
    ...
    gr.Examples(
        ...
        inputs=[seed_, sa32_, sa64_, id_length_, general_prompt, negative_prompt, prompt_array, style, model_type,
                files, G_height, G_width],
